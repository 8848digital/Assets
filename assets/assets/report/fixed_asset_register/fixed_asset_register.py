...
import frappe
from erpnext.erpnext.accounts.report.financial_statements import (
...
def get_data(filters):
    ...
    for row in data:
        if row["parent_asset"]:
            ...
            unique_parent_assets[custom_parent]["gross_purchase_amount"] += row[
                "gross_purchase_amount"
            ]
            ...
            unique_parent_assets[custom_parent]["depreciated_amount"] += row[
                "depreciated_amount"
            ]
...
def asset_category_filter(doctype, txt, searchfield, start, page_len, filters):
    ...
    query = (
        ...
        .select(ac.name)
        .where((acc.company_name == company) & (ac.name.like(f"%{txt}%")))
    )
    return query.run()
