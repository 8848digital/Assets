...
from erpnext.erpnext.controllers.accounts_controller import merge_taxes
from erpnext.erpnext.stock.doctype.purchase_receipt.purchase_receipt import (
...
def make_item_gl_entries(self, gl_entries, warehouse_account=None):
    ...
    def make_stock_received_but_not_billed_entry(item):
        ...
        if self.is_internal_transfer() and item.valuation_rate:
            outgoing_amount = abs(
                get_stock_value_difference(self.name, item.name, item.from_warehouse)
            )
    ...
    for d in self.get("items"):
        if (
            ...
            if (flt(d.valuation_rate) or self.is_return or d.is_fixed_asset) and flt(
                d.qty
            ):
...
def make_purchase_invoice(source_name, target_doc=None, args=None):
    from erpnext.erpnext.accounts.party import get_payment_terms_template
