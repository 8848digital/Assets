...
frappe.ui.form.on("Asset Maintenance", {
	setup: (frm) => {
		...
		frm.set_query("assign_to", "asset_maintenance_tasks", function (doc) {
			return {
				query: "assets.assets.doctype.asset_maintenance.asset_maintenance.get_team_members",
				filters: {
	...
	make_dashboard: (frm) => {
		if (!frm.is_new()) {
			frappe.call({
				method: "assets.assets.doctype.asset_maintenance.asset_maintenance.get_maintenance_log",
...
var get_next_due_date = function (frm, cdt, cdn) {
	...
	if (d.start_date && d.periodicity) {
		return frappe.call({
			method: "assets.assets.doctype.asset_maintenance.asset_maintenance.calculate_next_due_date",
			args: {
