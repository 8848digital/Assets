...
import erpnext
import frappe
from erpnext.erpnext.accounts.general_ledger import make_reverse_gl_entries
...
class Asset(AccountsController):
    ...
    def get_linked_item(self, purchase_doc_type, purchase_doc):
        ...
        for item in purchase_doc.items:
            if self.asset_quantity > 1:
                if (
                    item.base_net_amount == self.gross_purchase_amount
                    and item.qty == self.asset_quantity
                ):
                    return item.name
                ...
                if (
                    item.base_net_rate == self.gross_purchase_amount
                    and item.qty == self.asset_quantity
                ):
                    return item.name
    ...
    def validate_asset_and_reference(self):
        if self.purchase_invoice or self.purchase_receipt:
            ...
            doc_field = (
                "purchase_invoice" if self.purchase_invoice else "purchase_receipt"
            )
            ...
            if sum(existing_doc) >= next(
                (i.qty for i in reference_doc.items if i.item_code == self.item_code), 0
            ):
                frappe.throw(
    ...
    def validate_cost_center(self):
        if self.cost_center:
            ...
            if not frappe.get_cached_value(
                "Company", self.company, "depreciation_cost_center"
            ):
                frappe.throw(
                    _(
    ...
    def validate_in_use_date(self):
        ...
        for d in self.finance_books:
            if getdate(d.depreciation_start_date) < getdate(
    ...
    def validate_precision(self):
        ...
        if self.opening_accumulated_depreciation:
            self.opening_accumulated_depreciation = flt(
                self.opening_accumulated_depreciation,
                self.precision("opening_accumulated_depreciation"),
            )
    ...
    def validate_asset_values(self):
        ...
        if not self.calculate_depreciation:
            ...
            if self.is_fully_depreciated:
                frappe.throw(
                    _("Depreciation cannot be calculated for fully depreciated assets")
                )
            ...
            if linked_doc:
                ...
                if docstatus == 0:
                    frappe.throw(
                        ...
                        ).format(get_link_to_form(doctype_name, linked_doc))
                    )
    ...
    def validate_gross_and_purchase_amount(self):
        ...
        if (
            ...
            and self.gross_purchase_amount != self.purchase_amount
        ):
            error_message = _(
    ...
    def validate_asset_finance_books(self, row):
        ...
        if flt(row.expected_value_after_useful_life) < 0:
            frappe.throw(
                _(
                    "Row {0}: Expected Value After Useful Life cannot be negative"
                ).format(row.idx)
            )
    ...
    def validate_depreciation_start_date(self, row):
        if row.depreciation_start_date:
            if getdate(row.depreciation_start_date) < getdate(self.purchase_date):
                frappe.throw(
                    _(
                        "Row #{0}: Next Depreciation Date cannot be before Purchase Date"
                    ).format(row.idx)
                )

            if getdate(row.depreciation_start_date) < getdate(
                self.available_for_use_date
            ):
                frappe.throw(
                    _(
                        "Row #{0}: Next Depreciation Date cannot be before Available-for-use Date"
                    ).format(row.idx)
                )
        else:
            frappe.throw(
    ...
    def validate_total_number_of_depreciations_and_frequency(self, row):
        if row.total_number_of_depreciations <= 0:
            frappe.throw(
                _(
                    "Row #{0}: Total Number of Depreciations must be greater than zero"
                ).format(row.idx)
            )
        ...
        if row.frequency_of_depreciation <= 0:
            frappe.throw(
                _(
                    "Row #{0}: Frequency of Depreciation must be greater than zero"
                ).format(row.idx)
            )
    ...
    def get_value_after_depreciation(self, finance_book=None):
        if not self.calculate_depreciation:
            return flt(
                self.value_after_depreciation, self.precision("gross_purchase_amount")
            )
    ...
    def get_manual_depreciation_entries(self):
        ...
        records = (
            frappe.qb.from_(gle)
            .select(
                gle.voucher_no.as_("name"), gle.debit.as_("value"), gle.posting_date
            )
    ...
    def get_depreciation_rate(self, args, on_validate=False):
        ...
        if args.get("depreciation_method") == "Written Down Value":
            ...
            if args.get("rate_of_depreciation") and not flt(
                args.get("expected_value_after_useful_life")
            ):
                return args.get("rate_of_depreciation")
...
def make_journal_entry(asset_name):
    ...
    depreciation_cost_center, depreciation_series, company_cost_center = (
...
def get_values_from_purchase_doc(purchase_doc_name, item_code, doctype):
    ...
    matching_items = [
        item for item in purchase_doc.items if item.item_code == item_code
    ]

    if not matching_items:
        frappe.throw(
            _(f"Selected {doctype} does not contain the Item Code {item_code}")
        )
    ...
    return {
        ...
        "purchase_date": purchase_doc.get("bill_date")
        or purchase_doc.get("posting_date"),
        "gross_purchase_amount": flt(first_item.base_net_amount),
        "asset_quantity": first_item.qty,
        "cost_center": first_item.cost_center or purchase_doc.get("cost_center"),
        "asset_location": first_item.get("asset_location"),
        "purchase_receipt_item": (
            first_item.name if doctype == "Purchase Receipt" else None
        ),
        "purchase_invoice_item": (
            first_item.name if doctype == "Purchase Invoice" else None
        ),
    }
...
def update_existing_asset(asset, remaining_qty, new_asset_name):
    ...
    for row in asset.get("finance_books"):
        ...
        expected_value_after_useful_life = flt(
            (row.expected_value_after_useful_life * remaining_qty)
        ...
        current_asset_depr_schedule_doc.flags.should_not_cancel_depreciation_entries = (
            True
        )
...
def create_new_asset_after_split(asset, split_qty):
    ...
    for row in new_asset.get("finance_books"):
        ...
        new_asset_depr_schedule_doc.set_draft_asset_depr_schedule_details(
            new_asset, row
        )
