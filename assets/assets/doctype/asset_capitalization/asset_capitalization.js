...
frappe.provide("assets.assets");

assets.assets.AssetCapitalization = class AssetCapitalization extends erpnext.stock.StockController {
	setup() {
	...
	set_consumed_stock_items_tagged_to_wip_composite_asset(asset) {
		...
		if (asset) {
			return me.frm.call({
				method: "assets.assets.doctype.asset_capitalization.asset_capitalization.get_items_tagged_to_wip_composite_asset",
				args: {
					asset: asset,
				},
				callback: function (r) {
	...
	get_consumed_stock_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "assets.assets.doctype.asset_capitalization.asset_capitalization.get_consumed_stock_item_details",
				child: row,
				args: {
					args: {
						item_code: row.item_code,
	...
	get_consumed_asset_details(row) {
		...
		if (row && row.asset) {
			return me.frm.call({
				method: "assets.assets.doctype.asset_capitalization.asset_capitalization.get_consumed_asset_details",
				child: row,
				args: {
					args: {
						asset: row.asset,
	...
	get_service_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "assets.assets.doctype.asset_capitalization.asset_capitalization.get_service_item_details",
				child: row,
				args: {
					args: {
						item_code: row.item_code,
	...
	get_warehouse_details(item) {
		...
		if (item.item_code && item.warehouse) {
			me.frm.call({
				method: "assets.assets.doctype.asset_capitalization.asset_capitalization.get_warehouse_details",
				child: item,
				args: {
					args: {
						item_code: item.item_code,
...
cur_frm.cscript = new assets.assets.AssetCapitalization({ frm: cur_frm });
