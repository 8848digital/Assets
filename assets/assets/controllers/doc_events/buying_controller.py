...
def on_cancel(doc, method = None):
	if doc.doctype in ["Purchase Receipt", "Purchase Invoice"]:
		field = "purchase_invoice" if doc.doctype == "Purchase Invoice" else "purchase_receipt"
...
def validate_asset_return(doc):
		...
		purchase_doc_field = "purchase_receipt" if doc.doctype == "Purchase Receipt" else "purchase_invoice"
		not_cancelled_asset = []
		if doc.return_against:
			not_cancelled_asset = [
				d.name
				for d in frappe.db.get_all("Asset", {purchase_doc_field: doc.return_against, "docstatus": 1})
			]
...
def update_fixed_asset(doc, field, delete_asset=False):
	for d in doc.get("items"):
		if d.is_fixed_asset:
			...
			assets = frappe.db.get_all("Asset", filters={field: doc.name, "item_code": d.item_code})
...
def delete_linked_asset(doc):
	...
	asset_movement = frappe.db.get_value("Asset Movement", {"reference_name": doc.name}, "name")
